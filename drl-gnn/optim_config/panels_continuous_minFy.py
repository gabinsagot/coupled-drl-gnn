...
class panels_continuous_minFy:
    ...
    def __init__(self, path):
        ...
        self.x_min = np.array([-1.0] * self.n_panels)
        self.x_max = np.array([1.0] * self.n_panels)
        self.x_0 = np.array([0.0] * self.n_panels)
        self.action_scale_factor = 30.0  # scale factor for actions
        self.act_size = self.n_panels
        self.obs_size = self.act_size
        self.obs = np.zeros(self.obs_size)
        self.episode = 0

        # discrete action space values (for discrete action conversion)
        self.theta_step = 5  # step between discrete angles
        ...
        self.case = CaseParameters(
            path=f"environment_config/{self.name}.json",
            needs_cimlib=True,
            geometry_class=Panels,
        )

        # GNN params
        self.gnn_parameters = GNNParameters(path=f"environment_config/{self.name}.json")
    ...
    def create_env_trajectory(self, actions: List[float], ep: int):
        ...
        if not actions or len(actions) != self.n_panels:
        ...
        try:
            _ = create_trajectory(
                ...
                output_name=os.path.join(self.output_path, f"{self.name}_{ep}.xdmf"),
            )
        except Exception as e:
    ...
    def map_to_discrete_actions(self, continuous_actions: np.ndarray) -> List[int]:
        """
        ...
        if cont.size != self.n_panels:
            raise ValueError(f"Expected {self.n_panels} actions, got {cont.size}")
        indices = [
