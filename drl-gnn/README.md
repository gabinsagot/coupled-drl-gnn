# GRAPH-DRL: Combining Graph Neural Networks and Deep Reinforcement Learning

`graph-drl` is a library for combining the policy-based optimization (PBO) deep reinforcement learning library with graph neural networks (GNN) to solve optimizations of complex problems in computational fluid dynamics.

GNNs are used to model the spatial relationships and interactions between different components of the fluid dynamics system, while PBO is used to optimize the control policies for these systems. In short, PBO-GNN is a model-based deep reinforcement learning library where the models are GNNs for physics simulations (e.g., fluid dynamics, solid mechanics, etc.).

This repo provides an integration of the [pbo](https://github.com/theodore-michel/pbo) and [graph-physics](https://github.com/theodore-michel/graph-physics) librairies.
